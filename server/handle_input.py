...
def minimum_streaming_example():
    ...
    def ask_question(callback_fn):
        ...
        # =========== things above this should be innitialized only once
        # prompt = create_prompt(input)
        prompt = "Write a 50 word paragraph about whales"
        logTime(start_time, "Generated prompt")
        response = qa.run(prompt)
        return response
    ...
    return Response(generate(q), content_type='text/plain')
